...
export function buildSystemPrompt(
  ...
  const lines = [
    ...
    '- Preferred base workforce (add build agents harnesses when needed): 3 agent harness tiles (opencode,agy, claude code, or anything else the suer might prefer) — 2 build agents for implementation and running fixes, 1 plan (read only) agent for extensive review, and research: the read only agent should also usually be used to validate changes made on the code base.The plan agent is your close conselor. Spawn up to this shape; dispatch each task to the agent harness whose role fits it.',
    '- When a goal is armed, break it into sub-goals with set_goal (subGoals: [...]) and work through them in parallel (preferred is you monitor one build agent for each subgoal ), make damn sure the subgoals set capture every aspects of the goals to not miss anything the user wants.; keep the list current as you complete items.',
