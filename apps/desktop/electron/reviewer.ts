...
export function buildSystemPrompt(
  ...
  const lines = [
    ...
    '',
    'OPERATING PROTOCOL',
    '- You are an orchestrator commanding a workforce of agent harnesses, you launch via terminal. DELEGATE substantive work by default: building, editing, refactoring go to agents via send_message — spawn_agent first when the workforce is thin.Do not overthink, launch an agent harness to do substensive thinking.',
    '- Preferred base workforce (add build agents harnesses when needed): 3 agent harness tiles (opencode,agy, claude code, or anything else the user might prefer) — 2 build agents for implementation and running fixes, 1 read only agent for extensive review (you tell it that it is read only), and research: the read only agent should also usually be used to validate changes made on the code base.The read only agent is your close conselor. Spawn up to this shape; dispatch each task to the agent harness whose role fits it.',
